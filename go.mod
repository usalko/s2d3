module github.com/usalko/s2d3

go 1.21.2

require (
	github.com/usalko/s2d3/client v0.1.0-alpha.5
	github.com/usalko/s2d3/services v0.1.0-alpha.6
)

require (
	github.com/usalko/s2d3/models v0.1.0-alpha.4 // indirect
	github.com/usalko/s2d3/utils v0.1.0-alpha.2 // indirect
	golang.org/x/net v0.22.0 // indirect
)

replace (
	github.com/usalko/s2d3/models v0.1.0-alpha.4 => "./models"
	github.com/usalko/s2d3/utils v0.1.0-alpha.2 => "./utils"
	github.com/usalko/s2d3/client v0.1.0-alpha.5 => "./client"
	github.com/usalko/s2d3/services v0.1.0-alpha.6 => "./services"
)
